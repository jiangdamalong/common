...
func (h *HTaskManager) Start(threadNum int, chLen int) {
	...
	if cpuNum <= 0 {
		cpuNum = DefaultProcNum
	}
	var minThreadNum = MinRoutineNum

	/*if chLen/cpuNum > MinChannelLen {
		chLen = chLen / cpuNum
	} else {
		chLen = MinChannelLen
	}*/

	if threadNum/cpuNum > MaxRoutineNum {
		threadNum = threadNum / cpuNum
	} else {
		threadNum = MaxRoutineNum
	}
	...
	for i := uint64(0); i < h.divNum; i++ {
		h.hTasks[i].Start(threadNum, minThreadNum, chLen, false, cpuNum)
